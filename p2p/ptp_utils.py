...
def view_images(images, num_rows=1, offset_ratio=0.02, fp="test.png"):
...
def diffusion_step(model, controller, latents, context, t, guidance_scale, low_resource=False, generator=None, eta=0.0):
...
def text2image_ldm_stable(
    ...
    text_embeddings = pipe._encode_prompt(prompt, pipe.device, 1, False, "")
    ...
    for i, (t, null_text_t) in enumerate(pipe.progress_bar(zip(pipe.scheduler.timesteps, null_prompts))):
        context = [null_text_t.to(pipe.device).repeat(batch_size,1,1), text_embeddings]
        if not low_resource:
            context = torch.cat(context)
        latents = diffusion_step(pipe, controller, latents, context, t, guidance_scale, low_resource, generator)

    # Post-processing
    image = latent2image(pipe, latents)
